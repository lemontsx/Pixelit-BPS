...
fetch("/user", {
  ...
  .then((data) => {
    ...
    if (ge("role")) {
      ge("role").style.color = getRoleColor(userRole);
    }

    if (data.muted) {
      const mutePopup = document.createElement('div');
      mutePopup.style.cssText = `display: flex; align-items: center; position: fixed; top: 10%; right: 0%; background-color: #6f057a; box-shadow: inset 0 -0.365vw #570066, 3px 3px 15px rgba(0, 0, 0, 0.6); color: red; padding: 10px 20px; z-index: 1000; width: 340px; height: 150x; border-radius: 5px;`;
...
function createMessageHTML(message) {
  ...
  if (mediaUrlPattern.test(message.msg) && isValidUrl(message.msg)) {
    messageContent = `<img src="${escapeHTML(message.msg)}" class="chatImages" style="margin-top: 10px;" onclick="openModal('${escapeHTML(message.msg)}')" /> <br>`;
  } else {
  ...
  return `<div class="message">
    <div class="pfp">
      <img src="${escapeHTML(message.pfp)}" draggable="false" style="filter: drop-shadow(0 0 5px rgba(0, 0, 0, 0.5))" onerror="this.src='https://izumiihd.github.io/pixelitcdn/assets/img/blooks/logo.png';" />
    </div>
    <div class="messageContainer">
      <div class="usernameAndBadges">
        <div class="username" style="color: ${roleColor};">${username} ${timestampHTML}</div>
        <br>
